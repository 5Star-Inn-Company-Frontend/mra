...
Future<TerminalTransactions?> loadTerminalTransactions(int id) async {
  final token = await const FlutterSecureStorage().read(key: 'token');
  try {
    final response = await ApiService.dio.get('/terminals/transaction/$id',
        options: Options(headers: {'Authorization': 'Bearer $token'}));

    if (response.statusCode == 200 || response.statusCode == 201) {
      return TerminalTransactions.fromJson(response.data);
    }
